...
type Props = {
  params: Promise<{ islandSlug: string; houseSlug: string }>;
};
...
export default async function HouseDetailPage({ params }: Props) {
  const { islandSlug, houseSlug } = await params;
  const house = await getHouseBySlug(houseSlug);
  ...
  const jsonLd = {
    ...
    url: `${process.env.NEXT_PUBLIC_DOMAIN}/${islandSlug}/${house.slug}`,
    address: {
      "@type": "PostalAddress",
      addressLocality: house.location_area ? `${house.location_area}, Cyclades` : "Tinos",
  ...
  const backHref = `/${islandSlug}`;
  const backLabel = house.islands?.title || "All Properties";
            ...
            <div className="lg:col-span-2">
              ...
              <div className="mb-6 pb-6 border-b border-[#e6e2dd]">
                ...
                <p className="text-[#40484f] flex items-center gap-2 text-sm">
                  ...
                  {[house.location_area, (house as any).islands?.title, "Cyclades"]
                    .filter(Boolean)
                    .join(", ")}
                </p>
              </div>
